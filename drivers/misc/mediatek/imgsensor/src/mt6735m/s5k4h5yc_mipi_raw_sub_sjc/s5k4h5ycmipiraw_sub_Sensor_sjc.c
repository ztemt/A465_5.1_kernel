...
#include "s5k4h5ycmipiraw_sub_Sensor_sjc.h"

#define PFX "S5K4H5YC_sub_camera_sensor_sjc"
...
#define S5K4h5yc_sjc_sub_USE_AWB_OTP
#if defined(S5K4h5yc_sjc_sub_USE_AWB_OTP)
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K4H5YC_SJC_SUB_SENSOR_ID,
...
*************************************************************************/
#if defined(S5K4h5yc_sjc_sub_USE_AWB_OTP)
static kal_uint16 S5K4h5yc_sjc_sub_MIPI_read_otp_vendor_id(void)
{
    int i = 0;
    ...
    for (i = 0; i < 3; i++)
    {
        info_flag = read_cmos_sensor(0x3A04 + 8 * i);
        if ( (info_flag & 0x80) == 0)
        {
            break;
        }
    ...
    LOG_INF("%s,liukun ModuleHouseID = %x \n",__func__, ModuleHouseID);
    return ModuleHouseID;
}


static void SJC_SUB_SK4H5_read_OTP(void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        temp = read_cmos_sensor(0x3A04 + 8 * i);
        if ( (temp & 0x80) == 0)
        {
            wb_flag = i;
            break;
    ...
    if (i < 3)
    {
        current_R = read_cmos_sensor(0x3a06 + 8 * i);
        current_G = read_cmos_sensor(0x3a07 + 8 * i);
        current_B = read_cmos_sensor(0x3a08 + 8 * i); 
        golden_R  = read_cmos_sensor(0x3a09 + 8 * i);
        golden_G  = read_cmos_sensor(0x3a0A + 8 * i);
        golden_B  = read_cmos_sensor(0x3a0B + 8 * i); 
    }
    else
    {	 
    ...
    write_cmos_sensor(0x3a00, 0x00); //otp disable read
    LOG_INF("liukun [S5K4h5yc_sjc_sub_MIPI_read_otp_wb] current_R=0x%x, current_G=0x%x, current_B=0x%x\n", current_R, current_G, current_B);
    LOG_INF("liukun [S5K4h5yc_sjc_sub_MIPI_read_otp_wb] golden_R=0x%x, golden_G=0x%x, golden_B =0x%x\n", golden_R, golden_G, golden_B);
    ...
    //current_G = (current_Gr + current_Gb) / 2;
   		...
   		LOG_INF("liukun [S5K4h5yc_sjc_MIPI_read_otp_wb] r_ratio=0x%x, b_ratio=0x%x\n", r_ratio, b_ratio);      
}
static void SJC_SUB_SK4H5_apply_OTP()
{
   kal_uint16 R_GAIN = 0, 
   	          B_GAIN = 0,
   	          Gr_GAIN = 0, 
   ...
   LOG_INF("[S5K4h5yc_sjc] [S5K4h5yc_sjc_MIPI_algorithm_otp_wb1] R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", R_GAIN, B_GAIN, G_GAIN);    
}

static void S5K4h5yc_sjc_sub_MIPI_update_wb_register_from_otp(void)
{   

	LOG_INF("xuqian enter S5K4h5yc_sjc_sub_MIPI_update_wb_register_from_otp ========>>>>>>>>\n");
	struct Darling_SK4H5_otp_struct SK4H5_OTP = {0};
   SJC_SUB_SK4H5_read_OTP();
   SJC_SUB_SK4H5_apply_OTP();
	LOG_INF("xuqian exit S5K4h5yc_sjc_sub_MIPI_update_wb_register_from_otp ========!!!!!!!!\n");
	//return 0;
}
#endif
	...
	//while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
            ...
            #if defined(S5K4h5yc_sjc_sub_USE_AWB_OTP)
			  if(*sensor_id == 0x485B && S5K4h5yc_sjc_sub_MIPI_read_otp_vendor_id() == 0x10)
			  	*sensor_id = 0x485B + 5;
			  else
...
static kal_uint32 open(void)
{
	...
	sensor_init();
    #if defined(S5K4h5yc_sjc_sub_USE_AWB_OTP)
    S5K4h5yc_sjc_sub_MIPI_update_wb_register_from_otp();
    #endif
	spin_lock(&imgsensor_drv_lock);
...
UINT32 S5K4H5YC_SJC_MIPI_RAW_SUB_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
